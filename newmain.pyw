...
def grafiekConfig():
	...
	def GraphStart():
		try:
			...
			if arduinoAvailable:
				...
				if gemetenvals.get() == "CO2 en O2":
					...
					for t in range(1,aantalMetingen+1):
						...
						time.sleep(hoevaakMetingen)					
					lst = [x,coArray]
				elif gemetenvals.get() == "temperatuur":
					for t in range(1,aantalMetingen+1):
				...
				elif gemetenvals.get() == "geen":
					print(gemetenvals.get())
			...
			Plotknopf.configure(state="normal")
			erderetext = popuptext.cget("text")
			popuptext.configure(text=erderetext+"\nMeting gestopt met non-zero exit code (-1)")
	
	
	Plotknopf.configure(state="disabled")
	
	# de popup
	ConfigWindow = CTkFrame(master=win, height=340, width=500,border_color="white",border_width=1)
	ConfigWindow.place(x=800,y=200)
	...
	gemetenvals =  CTkOptionMenu(master=ConfigWindow, values=["geen","temperatuur","temperatuur en O2","temperatuur en CO2","temperatuur, CO2 en O2","CO2 en O2","O2","CO2"], width = 150, height=30)
	gemetenvals.place(y=200,x=30)
	snapfreqnaam = CTkLabel(master=ConfigWindow, text="per hoeveel metingen een foto gemaakt wordt")
	snapfreqnaam.place(y=230,x=30)
	snapfreq = CTkEntry(master=ConfigWindow, placeholder_text="wel een nummer A.U.B.", width=150, height=30, font=("Roboto", 10))
	snapfreq.place(y=260,x=30)
	toepasbtn = CTkButton(master=ConfigWindow, text="Start meting", command=InitiateGraph, width=150, height=20)
	toepasbtn.place(y=300,x=175)
...
Plotknopf = CTkButton(master=mainconfig, text="start meting", command = grafiekConfig)
