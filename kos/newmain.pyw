...
def lichtConfig():
	...
	def startLicht():
		...
		if choice == "Aan":
			lichtknop.configure(text="en er was licht.")
			package= "C----H----T----R-G-B-L1V-\n"
			ser.write(package.encode())
			lichtStatus1.configure(text="aan")
		elif choice == "Uit":
			lichtknop.configure(text="maar het bleef donker.")
			package= "C----H----T----R-G-B-L0V-\n"
			ser.write(package.encode())
			LichtStatus1.configure(text="uit")
		else:
			lichtknop.configure(text="en er was licht.")
			print("onder constructie help plis")
...
# licht knop
lichtknop = CTkButton(master=mainconfig,text="stel lichtsystemen in", command=lichtConfig)
...
lichtStatus2 = CTkLabel(master=mainconfig, text="0 minuten")
